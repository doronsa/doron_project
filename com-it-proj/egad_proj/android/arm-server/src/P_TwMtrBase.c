...
static e_ComResult e_SendMessegePrivate(CHANNEL_HANDLER p_handler, //[IN]
	int i_cmd, //[IN]
	...
	int i_RequestID) //[IN]
{
	...
	// check if need timeout handshake  not relavant in e_TaskSendReceive mode or in
//	if(/*!i_IsMasterMode(p) */ Mode==e_TaskReguler  &&  p->i_IsMasterSlaveNotSupported)
//	{
//		// wait until the  time bitween  previous  transmition  expired
//
//		if(p->UserHandShake.Method==e_HandshakeGuardTime )
//			WaitTimeBitweenMessage(p->SendTimeStamp,p->UserHandShake.i_HandShakeTimeBitweenMessage,p);
//
//		else// if other device not ready to and the handshake input signal is on
//			if(p->UserHandShake.Method==e_HandshakeCommand  && p->UserHandShake.ProcGetHandShakeState)
//			{
//				// wait untill other device will be ready
//				if(i_WaitHandShaikTransmitEnable(p->UserHandShake.ProcGetHandShakeState,p->UserHandShake.i_HandShakeTimeOutMS,p)==0)
//					return e_ComHandShakeTimeOut;
//			}
//
//	}
...
//		v_InitReciveState(p);
//
//	p->SendTimeStamp=ul_GetTimeStamp(p);
	...
	// add request to Q if it is request
//	if(p->i_IsMasterSlaveNotSupported  && (e_IsRequesType==e_ReqTypeRequest)&&(Mode==e_TaskReguler))
//		if(!i_PrAdd2ReqQ(p,&ProtocolHeader,ul_GetTimeStamp(p), i_TimeOutMillSec))
//		{
//			// the Queue is not mandatory , if Que full it mean that user will not get TimeOut Event if other side will not respond
//
//			//return eCom_Q_ReqFull;
//		}

		// the send callback null
//		if(!p->UserCallback.ProcSend)
//			return e_ComCallbackMissing;

		// send header
	    e = Protocol_SEND_COM_PROC(uart_fd, &ProtocolHeader, sizeof(ProtocolHeader));
		...
		if(i_size&&p_data)
		{
			//e=p->UserCallback.ProcSend(p->Handler,p_data,(unsigned short)i_size);
			e = Protocol_SEND_COM_PROC(uart_fd, p_data, (unsigned short)i_size);
			printf("send data %s\n",p_data);
			if(e!=e_ComOk)
				return e;// send fail
...
#ifdef _WIN32_DEBUG
	memset(DebugMssgBuffer, 0, DebugMssgBuffer_Length);
	strcpy(DebugMssgBuffer, "\np->RequestCnt=\0");
	ltoa(ProtocolHeader->RequestCount, DebugMssgBuffer + strlen(DebugMssgBuffer), 10);
	printf(DebugMssgBuffer);
#endif
}
	...
	)
{
	...
	St_UserData  *p = p_FindChannelByCOMHndl(p_handler);
...
#ifdef _WIN32_DEBUG
	memset(DebugMssgBuffer, 0, DebugMssgBuffer_Length);
	strcpy(DebugMssgBuffer, "\np->Header.RequestCnt=\0");
	ltoa(p->Header.RequestCount, DebugMssgBuffer + strlen(DebugMssgBuffer), 10);
	printf(DebugMssgBuffer);
#endif
...
//	_I_AM_ALIVE_EVENT
	...
	e=e_SendMessegePrivate(p_handler,
						   ...
						   e_TaskReguler,
						   eRT_OK,
						   1/*DEFAULT_REQUEST_ID*/          /*Use internal counter*/);
	...
	)
{
	...
	for(i=0;i< size;i++)
	{
		Privatev_OnByteReceive(p_handler,p_Characters[i]);
