	...
	char				*pszPath )
{
	...
	if( ( nFd = open( pszPath, O_RDWR | O_EXCL | O_NONBLOCK ) ) < 0 )
	{
       ...
       //non-blocking  mode
        //int saved_flags = fcntl(nFd, F_GETFL);
       // fcntl(nFd, F_SETFL, saved_flags & ~O_NONBLOCK);
        ...
        if( FD_ISSET( nDeviceFid, &tagOutput ) )
	{
	  ...
	  FD_SET( nDeviceFid, &tagOutput );
           ...
           if(nRetVal <= 0)
	   {
           ...
           if( FD_ISSET( nDeviceFid, &tagOutput ) )
	   {
               len = write(nDeviceFid, pbyCmd + sum, tagWriteBytes - sum);
               sum += len;
           } 
           else
               return ( ERR_SII_API_TIMEOUT );

        }while(sum < tagWriteBytes );
       }
       ...
       if( FD_ISSET( nDeviceFid, &tagOutput ) )
	{
             ...
             //  if(sum == (int)tagWriteBytes )
              //    break;
                 ...
                 usleep(1000 * 150);
               printf( "%d %d %d %d\n",len, sum , tagWriteBytes ,acc); 
              }while(sum < tagWriteBytes && acc < 10);
