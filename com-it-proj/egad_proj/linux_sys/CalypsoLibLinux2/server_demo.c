  	...
  	while(1)
  	{
  		if (ExchangeDataCB.recive_data == 1)
  		{
  			...
  			if (ExchangeDataCB.datalen>0)
  			{
   				...
   				//for test
   				//memcpy(&resp,(void*)(p_ObjectOut),ExchangeDataCB.datalen);
  				//print_buffer((unsigned char*)p_ObjectOut,ExchangeDataCB.datalen);
  			}
  			else
  			{
  				p_ObjectOut = NULL;
...
int main( void )
{
	...
	int res=0;
	//st_InitResource * InitResource = (st_InitResource*)malloc(sizeof(st_InitResource));
    //init calypso
